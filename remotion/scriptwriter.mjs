...
const SCHEMA_AVATAR = `AvatarSpec = {
  ...
  script: string,  // ПОЛНЫЙ монолог для озвучки реальным человеком (HeyGen). НАЧИНАЕТСЯ с hook.
                   // Структура: крючок -> личная история/ценность (что делает продукт) -> мягкий призыв.
                   // Устная речь: короткие предложения, разговорно, ~35-45 слов (~13-16 сек).
                   // БЕЗ эмодзи, без хэштегов, без «ссылка в описании» — это произносится ВСЛУХ.
...
const sysAvatar =
  ...
  `Текст пишется ДЛЯ ПРОИЗНЕСЕНИЯ ВСЛУХ. Главное — мощный эмоциональный ХУК в первой секунде (удержание!). ` +
