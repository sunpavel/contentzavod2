...
const SCHEMA_AVATAR = `AvatarSpec = {
  hook: string,    // ЭМОЦИОНАЛЬНЫЙ крючок — ПЕРВАЯ фраза монолога (первые ~2 сек). Разрыв шаблона /
                   // острая боль / смелое заявление. Коротко, разговорно, цепляет мгновенно.
  script: string,  // ПОЛНЫЙ монолог для озвучки реальным человеком (HeyGen). НАЧИНАЕТСЯ с hook.
                   // Структура: крючок -> одно предложение ценности (что делает продукт) -> мягкий призыв.
                   // ЖЁСТКО: МАКСИМУМ 40 слов, 3-4 КОРОТКИХ предложения, ~15 секунд речи. КОРОЧЕ = ЛУЧШЕ.
                   // Устная речь, разговорно. БЕЗ эмодзи, хэштегов и «ссылка в описании» — это говорится ВСЛУХ.
  ctaTitle: string,// финальная плашка: призыв в 2 строки через \\n, БЕЗ @-хэндла
  accent: string   // hex: бирюза #14C7C0 / зелёный #34D399 / янтарь #F4A623
}`;
...
const sysAvatar =
  `Каждый ролик — это AvatarSpec по схеме: ${SCHEMA_AVATAR} ` +
  `Формат — РЕАЛЬНЫЙ ЧЕЛОВЕК за столом говорит в камеру (UGC), на экране всплывает приложение. ` +
  `Текст пишется ДЛЯ ПРОИЗНЕСЕНИЯ ВСЛУХ и должен быть КОРОТКИМ: максимум 40 слов на весь монолог (~15 сек). ` +
  `Главное — мощный эмоциональный ХУК в первой секунде (удержание!), затем одно предложение про продукт и призыв. ` +
  `Тон живой, личный, как настоящий отзыв. Опирайся на структуру и хук залетевшего тренда из брифа.`;
...
const SPECNAME = FORMAT === "avatar" ? "AvatarSpec" : FORMAT === "creator" ? "CreatorSpec" : "ReelSpec";
...
const system =
  ...
  (FORMAT === "avatar" ? sysAvatar : FORMAT === "creator" ? sysCreator : sysDemo) +
  ...
  `Верни СТРОГО JSON-массив из ${N} ${SPECNAME}. Кириллица. Без медицинских обещаний.`;

const userMsg = `БРИФ (из трендового видео):\n${brief}\n\nВерни массив из ${N} ${SPECNAME}.`;
